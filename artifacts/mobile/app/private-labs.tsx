  ...
  return (
    ...
    >
      ...
      <View style={styles.cardHeader}>
        ...
        {/* Expand arrow only when searching */}
        {isSearching && (
          <Text style={styles.expandArrow}>{expanded ? "▲" : "▼"}</Text>
        )}
      </View>

      {/* When not searching: show test count per field */}
      {!isSearching && (
      ...
      {/* When searching: show matching tests (collapsed by default, expand on tap) */}
      {isSearching && (
        <View style={styles.matchSummary}>
          <Text style={styles.matchCount}>{matches.reduce((a, g) => a + g.parameters.length, 0)} اختبار مطابق</Text>
        </View>
      )}
            ...
            return (
              <View key={gi} style={styles.matchGroup}>
                ...
                {grp.parameters.map((p, pi) => (
                  <Text key={pi} style={styles.paramText}>
                    • {p}
                  </Text>
                ))}
              </View>
...
const styles = StyleSheet.create({
  ...
  // Match summary (when searching, collapsed)
  matchSummary: {
    paddingHorizontal: 14,
    paddingBottom: 10,
    alignItems: "flex-end",
